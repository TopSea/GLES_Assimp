...
//        LOGE("path: %s",filename.c_str());

        int inDir = filename.find_last_of('\\');
        if (inDir > 0) {
            filename = filename.substr(inDir);
            filename[0] = '/';
            filename = "yuanshen_77/Texture" + filename;
        }

		string dir = directory;
		filename = dir.append("/").append(filename);
//        LOGE("path2: %s",filename.c_str());
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        LOGE("loadMaterialTextures done");
        return textures;
    }
};
